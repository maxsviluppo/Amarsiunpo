...
$path = "C:\Users\Max\Downloads\A Codici Main\SoulMatch-main\SoulMatch-main\src\App.tsx"
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -like "*const SubscriptionSuccessPage = () => {*") {
        $startLine = $i
        break
    }
}
for ($i = $startLine; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -like "*const SecurityOverlay = ({ status, onClose }:*") {
        $endLine = $i - 1
        break
    }
}
...
if ($startLine -gt 0 -and $endLine -gt $startLine) {
    for ($i = 0; $i -lt $startLine; $i++) { $null = $newLines.Add($lines[$i]) }
...
"@
    $null = $newLines.AddRange($cleanCode -split "`r`n")
    for ($i = $endLine + 1; $i -lt $lines.Count; $i++) { $null = $newLines.Add($lines[$i]) }
    
    [System.IO.File]::WriteAllLines($path, $newLines)
} else {
    Write-Error "Could not find start/end lines"
}
